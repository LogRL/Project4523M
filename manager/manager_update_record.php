...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="manager_update_record.php">Update Order records</a>
          </li>
  ...
  <div class="filter-container">
    <input type="text" id="filterInput" class="form-control" placeholder="Filter orders...">
  </div>
  <div class="container">
    <div class="row">
  ...
  <div class="accordion-container">
    <div class="accordion" id="accordionFlushExample">
      <?php foreach ($orderId as $key => $orderid):
        $mkey = $key ?>
        <div class="accordion-item" data-order-id="<?php echo $orderid; ?>"
          data-order-status="<?php echo $orderStatus[$mkey]; ?>">
          <h2 class="accordion-header" id="flush-headingOne">
            <button class="btn btn-light accordion-button collapsed" type="button" data-bs-toggle="collapse"
              data-bs-target="#flush-collapse<?php echo "$mkey" ?>" aria-expanded="false"
              aria-controls="flush-collapse<?php echo "$mkey" ?>">
              <div><br>
                Order: <?php echo "$orderid" ?> <br>
                Manager ID: <?php if ($smId[$mkey] === null) {
                  echo "NULL";
                } else {
                  echo "$smId[$mkey]";
                } ?><br>
                Manager's Contact Name:
                <?php if ($smId[$mkey] === null) {
                  echo "NULL";
                } else {
                  $smIndex = $smId[$mkey] - 1;
                  echo "$contactName[$smIndex]";
                } ?><br>
                ...
                <div>
                  ...
                  <?php
                  switch ($orderStatus[$mkey]) {
                    case 'waiting to process':
                      echo "<p>Order Status: <span class='text-secondary'>$orderStatus[$mkey]</span></p>";
                      break;
                    case 'accepted':
                     echo "<p>Order Status: <span class='text-success'>$orderStatus[$mkey]</span></p>";
                      break;
                    case 'rejected':
                      echo"<p>Order Status: <span class='text-danger'>$orderStatus[$mkey]</span></p>";
                      break;
                    case 'cancel':
                      echo"<p>Order Status: <span class='text-warning'>$orderStatus[$mkey]</span></p>";
                      break;
          ...
          <div id="flush-collapse<?php echo "$mkey" ?>" class="accordion-collapse collapse"
              ...
              <tbody>
                ...
                foreach ($orderItemIndex as $mykey => $item):
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="col-sm-2 align-self-start"><img src="<?php echo "$itemImage[$item]" ?>"
                          class="img-thumbnail" alt="..." width="200" height="200"></div>
                    </td>
                    <td><?php echo "$itemName[$item]" ?></td>
                    <td><?php echo "$quantity[$mykey]" ?></td>
                    <td><?php $sum = $price[$item] * $quantity[$mykey];
                    echo $sum; ?></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
  ...
  <script>
    document.getElementById('filterInput').addEventListener('input', function () {
      ...
      items.forEach(function (item) {
        var orderId = item.getAttribute('data-order-id').toLowerCase();
        var orderStatus = item.getAttribute('data-order-status').toLowerCase();
